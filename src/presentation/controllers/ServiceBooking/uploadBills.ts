...
export const uploadBillsHandler = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!Array.isArray(invoices) || invoices.length === 0) {
      res.status(HttpStatus.BAD_REQUEST).json({ message: "No invoice images provided." });
      return;
    }
    ...
    res.status(HttpStatus.CREATED).json({ message: "Invoice images uploaded successfully." });
  } catch (error) {
    console.error("Error uploading invoice bills:", error);
    res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ message: "Failed to upload invoice images." });
  }
};
