...
export const GetServiceProviderBookServiceHandler = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (!serviceProviderId) {
      res
        ...
        .json({ error: "Service provider ID is required." });
      return;
    }

    const getBookService = container.resolve(GetBookService);
    const { service, count } =
      await getBookService.ServiceProviderBookedServices(
        serviceProviderId,
        skip,
        limit
      );
    ...
    console.error(
      "Error in GetBookServiceHandler:",
