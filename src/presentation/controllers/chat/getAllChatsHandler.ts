...
import { GetAllChatsUseCase } from '../../../application/use-case/chat/GetAllChatsUseCase';
...
export const getAllChatsHandler = async (req: Request, res: Response) => {
  try {
    ...
    if (serviceProviderId) {
      chats = await getAllChatsUseCase.getServiceProviderChats(serviceProviderId);
    } else if (userId) {
      chats = await getAllChatsUseCase.getUserChats(userId);
    } else {
       res.status(HttpStatus.BAD_REQUEST).json({ message: 'Missing userId or serviceProviderId' });
       return
    }
