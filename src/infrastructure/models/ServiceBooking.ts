...
const ServiceBookingSchema = new Schema<IServiceBooking>({
    ...
    serviceStatus: { type: String,default:"pending", enum: ['pending', 'in-progress', 'completed', 'cancelled'], required: true },
    paymentType: { type: String,default:"pending", enum: ['cash', 'card', 'online','pending'], required: true },
    paymentStatus: { type: String, default:"pending", enum: ['pending', 'paid', 'failed','pending'], required: true },
