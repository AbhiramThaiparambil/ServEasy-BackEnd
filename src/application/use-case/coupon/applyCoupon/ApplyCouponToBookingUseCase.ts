...
import { IApplyCouponToBookingUseCase } from "./IApplyCouponToBookingUseCase";
...
export class ApplyCouponToBookingUseCase implements IApplyCouponToBookingUseCase {
  ...
  async execute({ bookingId, couponCode }: { bookingId: string; couponCode: string }) {
    ...
    if (coupon.minOrderAmount && booking.payment.total < coupon.minOrderAmount) {
      throw new Error("Order total does not meet minimum requirement for this coupon");
    }

    const alreadyUsed = await this.couponRepo.hasUserUsedCoupon(coupon.code, booking.userId.toString());
    ...
    await this.couponRepo.markUsedByUser(coupon.code, booking.userId.toString());
