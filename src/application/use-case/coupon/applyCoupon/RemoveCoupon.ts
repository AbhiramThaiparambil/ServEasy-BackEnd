...
export class RemoveCouponToBookingUseCase implements IRemoveCouponToBookingUseCase {
  constructor(
    @inject(ServiceBookingRepository) private bookingRepository: IServiceBookingRepository,
    @inject(REPOSITORY_TOKENS.CouponRepository) private couponRepo:ICouponRepository
  ) {}
  ...
  async execute(bookingId: string): Promise<any> {
    ...
    const updatedBooking = await this.bookingRepository.removeCouponAndUpdatePayment(bookingId);
